 ...
 */ class ClientPool {
    ...
    async terminate() {
        ...
        if (this.opCount > 0) {
            (0, logger_1.logger)('ClientPool.terminate', /* requestTag= */ null, 'Waiting for %s pending operations to complete before terminating (%s)', this.opCount, this.lazyLogStringForAllClientIds);
 ...
 */ class LazyLogStringForAllClientIds {
    ...
    toString() {
        return `${this.activeClients.size} active clients: {` + this.logStringFromClientIds(this.activeClients.keys()) + '}, ' + `${this.failedClients.size} failed clients: {` + this.logStringFromClientIds(this.failedClients) + '}';
    }
    logStringFromClientIds(clients) {
        return Array.from(clients).map((client)=>{
            var _a;
            return (_a = this.clientIdByClient.get(client)) !== null && _a !== void 0 ? _a : '<unknown>';
        }).sort().join(', ');
    }
